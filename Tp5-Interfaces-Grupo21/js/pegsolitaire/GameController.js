...
function onMouseUp(e) {
    ...
    if (dest && board.isValidMove(lastClickedChip.cellRow, lastClickedChip.cellCol, dest.row, dest.col)) {
        ...
        if (chips.length === 1) {
            ...
            if(board.getChipAt(board.rows / 2, board.cols / 2) !== null){
                console("Soy un pedazo");
                endGame("La ultima ficha quedo en el medio. Ganaste, fin del juego", true);
                return;
            }
            else{
                endGame("La ultima ficha no quedo en el medio. Perdiste, fin del juego", false);
                return;
            }
